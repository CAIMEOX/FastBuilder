...
请勿选择购买盗版，我们的官方联系方式在主页给出，如因购买盗版而带来的任何损失(包括但不限于丢失数据等)我们拒绝承担责任。
...
1. 从[**Termux官网提供的下载渠道**](https://f-droid.org/packages/com.termux/)下载软件Termux，当前版本：[0.84](https://f-droid.org/repo/com.termux_84.apk)
...
2. 执行`npm i supervisor -g`（在macOS及iOS上可能需要执行`sudo npm i supervisor -g`才能成功，会要求输入密码，iOS需前往Cydia安装sudo）
   - 2.1 iOS设置技巧
   -   -  2.1.1 安装Sudo后，打开Terminal
   -   -  2.1.2 执行`su`，会要求输入密码，默认密码`alpine`，注意，输入时是不显示你输入了多少字的！输入完毕后按下换行即可
...
通常情况下原因是内存不足，可以运行<code>node SubF --max-old-space-size=2048</code>来给FastBuilder Pro分配2G内存，单位MB，根据需要调整
   -   如通过安卓傻瓜式安装且使用`cd ~/ && ./fbpro`命令启动，则无需另行分配内存
